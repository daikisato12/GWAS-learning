...
例えば, コーヒー摂取が肺がんに及ぼす影響を知りたい. この時, 喫煙というコーヒー摂取と肺がん罹患の両者に影響する交絡因子が考えられる. これまでであれば, 喫煙量を群間でコントロールした上でコーヒー摂取/非摂取群を作り, 肺がんに及ぼす影響を調べていた. しかしMRでは, コーヒー摂取と強く相関する遺伝的変異をGWASで特定した上で, その遺伝子型で2群を分け, 肺がんリスクを群間で比較する, という研究になる. コーヒー摂取 (あるいは他の交絡要因) と喫煙が遺伝的な相関を持たない限り, コーヒー摂取傾向のみが肺がんに与える影響を推定できると考えられる. <br>
...
### 関連項目
- 線形混合モデル (LMM: Linear Mixed Model): SNPの効果を固定効果, 性別や年齢 (, 年齢^2, 場合によってはBMIも)を共変量, 遺伝的な集団構造 (e.g. PC1–10) を変量効果として入れて解析を行うスタンダードなモデル. `PLINK`で解析可能. 基本的に前述の手法は全てこの改良版. 個体数N, 計M個のSNPsがあった時, 個体の表現型yは, 各SNPxの固定効果β (これを求めるのが目的となる), サンプル間の遺伝的距離 (kinship matrixあるいはgenetic relationship matrix (GRM)と呼ばれる) に基づくpolygenicな変量効果g, エラー項eで表される. <br>
